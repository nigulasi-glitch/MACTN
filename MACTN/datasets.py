...
def print_dataset_info(dataset, dataset_name, target):
    print(f"\n{'=' * 60}")
    ...
    for node_type in data.node_types:
        if hasattr(data[node_type], 'x') and data[node_type].x is not None:
            feat_dim = data[node_type].x.shape[1] if len(data[node_type].x.shape) > 1 else 1
            num_nodes = data[node_type].num_nodes
            print(f"  {node_type}: {num_nodes}个节点, 特征维度: {feat_dim}")
        else:
            print(f"  {node_type}: {data[node_type].num_nodes}个节点, 无特征")
    ...
    if hasattr(data[target], 'y') and data[target].y is not None:
        num_classes = len(torch.unique(data[target].y))
        print(f"\n目标节点类型: {target}")
        print(f"类别数量: {num_classes}")
        print(f"标签分布: {torch.bincount(data[target].y).tolist()}")
    ...
    for node_type in data.node_types:
        if hasattr(data[node_type], 'x') and data[node_type].x is not None:
            ...
            if features is not None and len(features) > 0:
                ...
                print(f"    稀疏度: {(features == 0).float().mean():.4f}")
...
if __name__ == '__main__':
    datasets = ['dblp', 'imdb', 'acm', 'aminer', 'freebase']

    print("开始分析所有数据集的特征维度...")
    for dataset_name in datasets:
        try:
            dataset, metapaths, target = get_dataset(dataset_name)
