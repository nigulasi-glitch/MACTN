import torch
from augment.preprocess import remove_self_loop, normalize_adj_from_tensor
...
def get_top_k(sim_l, sim_h, k1, k2):
    ...
    kg_pos = torch.sparse.FloatTensor(
        k_indices_pos,
        torch.ones((len(k_indices_pos[0]))).to(sim_l.device),
        ([len(sim_l), len(sim_l)])
    )
    kg_neg = torch.sparse.FloatTensor(
        k_indices_neg,
        torch.ones((len(k_indices_neg[0]))).to(sim_l.device),
        ([len(sim_l), len(sim_l)])
    )
