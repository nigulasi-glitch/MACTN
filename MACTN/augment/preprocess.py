...
def mask_feature(feat, adj_mask):
    ...
    for j in mask_idx:
        feats_data[j] = 0
    mask_feat = torch.sparse.FloatTensor(
        torch.LongTensor([feats_coo.row.tolist(), feats_coo.col.tolist()]),
        torch.FloatTensor(feats_data.astype(np.float64)))
...
def txt_to_coo(dataset, relation):
    ...
    return torch.sparse.FloatTensor(
        torch.LongTensor([row.tolist(), col.tolist()]),
...
def remove_self_loop(adjs):
    ...
    for i in range(len(adjs)):
        ...
        adj = torch.sparse.FloatTensor(
            adj.indices()[:, diag_index],
            adj.values()[diag_index],
            adj.shape).coalesce()
        adjs_.append(adj)
    return adjs_

def add_self_loop_and_normalize(indices, values, nnodes):
    i_indices = torch.stack(
        (torch.tensor(range(0, nnodes)), torch.tensor(range(0, nnodes))), dim=0).to(indices.device)
...
def sparse_tensor_add_self_loop(adj):
    ...
    index = torch.stack(
        (torch.tensor(range(node_num)), torch.tensor(range(node_num))), dim=0).to(adj.device)
    values = torch.ones(node_num).to(adj.device)
    adj_new = torch.sparse.FloatTensor(
        torch.cat((index, adj.indices()), dim=1),
        torch.cat((values, adj.values()), dim=0),
        adj.shape)
    return adj_new
...
def pre_filter(feat, adjs, lg_l, lg_h, nlayer, device):
    ...
    adjs_l_filter, adjs_h_filter, lg_l_filter, lg_h_filter = [[] for _ in range(num_sub_graph)], [[] for _ in range(
        num_sub_graph)], [], []
...
def pos_samplering(seed, pos, device):
    ...
    pos_graph = torch.sparse.FloatTensor(
        torch.stack((pos_0_in_all, pos_1_in_all), dim=0),
        torch.ones(len(pos_0_in_all)),
        ([len(all_index), len(all_index)])).coalesce()
